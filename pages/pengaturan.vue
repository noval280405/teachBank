      ...
      <div class="space-y-5">
        ...
        <article v-for="(tugas, index) in penugasanTerurut" :id="`penugasan-${tugas.id}`" :key="tugas.id" :class="['scroll-mt-24 overflow-hidden rounded-2xl border-2 bg-white shadow-sm transition-all duration-300 dark:bg-slate-800', warnaJenjang(tugas.jenjang).card]">
          ...
          <div class="space-y-6 p-5 sm:p-6">
            ...
            <div><div class="mb-3"><h3 class="text-sm font-bold">Mata pelajaran per kelas</h3><p class="text-xs text-slate-500">Atur mapel berbeda untuk setiap kelas yang Anda ajar.</p></div><div v-if="!tugas.kelasAjar.length" class="rounded-xl border border-dashed border-slate-300 py-6 text-center text-xs text-slate-400 dark:border-slate-700">Pilih kelas terlebih dahulu.</div><div v-else class="space-y-3"><section v-for="kelas in [...tugas.kelasAjar].sort((a,b) => a-b)" :key="`mapel-${tugas.id}-${kelas}`" class="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><div class="mb-3 flex items-center justify-between"><div class="flex items-center gap-2"><span :class="['grid h-8 w-8 place-items-center rounded-lg text-sm font-bold', warnaJenjang(tugas.jenjang).icon]">{{ kelas }}</span><div><h4 class="text-sm font-bold">Kelas {{ kelas }}</h4><p class="text-[10px] text-slate-400">{{ mapelKelas(tugas, kelas).length }} mapel dipilih</p></div></div><button v-if="mapelKelas(tugas, kelas).length" type="button" @click="salinMapelKeSemuaKelas(tugas, kelas)" class="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-500 hover:text-brand-600" title="Terapkan mapel ini ke semua kelas"><AppIcon name="copy" class="h-3 w-3" />Terapkan ke semua</button></div><div class="flex flex-wrap gap-2"><button v-for="mapel in opsiMapel(tugas.jenjang, tugas)" :key="`${kelas}-${mapel}`" type="button" @click="toggleMapelKelas(tugas, kelas, mapel)" :class="['inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold transition', mapelKelas(tugas, kelas).includes(mapel) ? 'border-brand-600 bg-brand-600 text-white' : 'border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-300']"><AppIcon :name="mapelKelas(tugas, kelas).includes(mapel) ? 'check' : 'plus'" class="h-3.5 w-3.5" />{{ mapel }}</button></div></section></div><div class="mt-3 flex gap-2"><input v-model="tugas.mapelBaru" @keyup.enter.prevent="tambahMapel(tugas)" type="text" placeholder="Tambah pilihan mata pelajaran lainnya..." class="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-brand-500 dark:border-slate-700 dark:bg-slate-900/40" /><button type="button" @click="tambahMapel(tugas)" class="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-4 text-sm font-semibold text-white dark:bg-slate-700"><AppIcon name="plus" class="h-4 w-4" />Tambah pilihan</button></div></div>
          </div>
        </article>
...
const buatPenugasan = (data = {}) => {
  const kelasAjar = data.kelasAjar || []
  const mapelPerKelas = data.mapelPerKelas || Object.fromEntries(kelasAjar.map(kelas => [kelas, [...(data.mapelAjar || [])]]))
  return { id: data.id || `tugas_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, jenjang: data.jenjang === 'SMK' || data.jenjang === 'SMA' ? 'SMA/SMK' : data.jenjang || 'SD', namaSekolah: data.namaSekolah || '', kelasAjar, mapelPerKelas, mapelAjar: data.mapelAjar || [...new Set(Object.values(mapelPerKelas).flat())], customMapel: data.customMapel || [], mapelBaru: '' }
}
const form = ref({ penugasan: [buatPenugasan()] })
const jumlahKelas = (jenjang) => jenjang === 'SD' ? 6 : 3
const opsiMapel = (jenjang, tugas) => [...new Set([...mapelUmum, ...(mapelJenjang[jenjang] || []), ...(tugas?.customMapel || [])])]
...
const salinMapelKeSemuaKelas = (tugas, sumberKelas) => { const mapel = [...mapelKelas(tugas, sumberKelas)]; tugas.kelasAjar.forEach(kelas => { tugas.mapelPerKelas[kelas] = [...mapel] }); sinkronMapel(tugas) }
const sinkronMapel = (tugas) => { tugas.mapelAjar = [...new Set(tugas.kelasAjar.flatMap(kelas => mapelKelas(tugas, kelas)))] }
const tambahMapel = (tugas) => { const nama = tugas.mapelBaru?.trim(); if (nama && !opsiMapel(tugas.jenjang, tugas).includes(nama)) tugas.customMapel.push(nama); tugas.mapelBaru = '' }
...
const simpanPengaturan = async () => {
  ...
  form.value.penugasan.forEach(sinkronMapel)
  const invalid = form.value.penugasan.some(t => !t.namaSekolah.trim() || !t.kelasAjar.length || t.kelasAjar.some(kelas => !mapelKelas(t, kelas).length))
  if (invalid) { alert('Lengkapi nama sekolah dan pilih minimal satu mata pelajaran untuk setiap kelas yang diajar.'); return }
